...
our @EXPORT_OK =qw(read_whole_file);
    ...
    until (eof($fh)) {
        ...
        if (defined($defined_blocks{$block_type})) {
           $pcapng_file{$counter}=$defined_blocks{$block_type}(\$block_length,\$block_data);
            ...
            print "$block_type -- I have no idea what this is!!!\n";
        }
    }
    close($fh);
    return \%pcapng_file;
...
sub read_enhanced_packet_block {
    ...
    my $padding=(4-($block{'block_original_packet_length'}%4))%4;
    $block_location=20+$block{'block_original_packet_length'}+$padding;
    if ($block_location < $$block_length) {
        $block{'block_options'}=&get_options($block_data,$block_location,$block_length,$block_code);
    }
    #    print Dumper %block;
...
sub get_options {
    ...
    my $code=100000;
    my %options=();
    print "Getting some options! $block_location and $$block_length\n";
    if ($block_location <= ($$block_length)) {
        until ($block_location >= ($$block_length) or $code == 0) {
